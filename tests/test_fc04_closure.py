...
def closure_data() -> tuple[dict, dict]:
    return build()


@pytest.mark.slow
def test_closure_covers_all_5b_unresolved_targets(closure_data: tuple[dict, dict]) -> None:
    inventory, reconciliation = closure_data
    assert validate() == []
...
@pytest.mark.slow
def test_closure_preserves_evidence_and_safety_boundaries(
    closure_data: tuple[dict, dict],
) -> None:
    inventory, _ = closure_data
...
@pytest.mark.slow
def test_closure_artifacts_rebuild_deterministically(
    closure_data: tuple[dict, dict],
) -> None:
    expected = closure_data
