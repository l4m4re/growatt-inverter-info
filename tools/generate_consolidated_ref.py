...
DOC_DIR = PROJECT_ROOT / "sources"
VENDOR_TABLE_PATH = (
    DOC_DIR / "vendor" / "growatt-v1.24-tables.json"
)
BEST_GUESS_PATH = DOC_DIR / "curated" / "growatt-registers-best-guess.json"
HA_LOCAL_PATH = DOC_DIR / "runtime" / "ha-local-registers.snapshot.json"
OPENINVERTER_PATH = DOC_DIR / "external" / "openinverter-gateway-registers.snapshot.json"
INVERTER_TO_MQTT_PATH = DOC_DIR / "external" / "inverter-to-mqtt-registers.snapshot.json"
GRAPH_PATH = PROJECT_ROOT / "knowledge" / "graph" / "register-graph.gpickle"
GROTT_LAYOUTS_PATH = DOC_DIR / "external" / "grott-register-layouts.snapshot.json"
OUTPUT_PATH = PROJECT_ROOT / "knowledge" / "audit" / "consolidated-register-reference.json"
...
def generate_from_graph(graph: nx.MultiDiGraph) -> Dict[str, Any]:
    ...
    source_files = {
        "register_graph": {
            "path": str(GRAPH_PATH.relative_to(PROJECT_ROOT)),
...
def generate_payload_from_sources() -> Dict[str, Any]:
    ...
    source_files: Dict[str, Any] = {
        "vendor_tables": {
            "path": str(VENDOR_TABLE_PATH.relative_to(PROJECT_ROOT)),
            "sha256": sha256sum(VENDOR_TABLE_PATH),
        },
        "best_guess_json": {
            "path": str(BEST_GUESS_PATH.relative_to(PROJECT_ROOT)),
            "sha256": sha256sum(BEST_GUESS_PATH),
        },
    }
    if ha_data:
        source_files["ha_local"] = {
            "path": str(HA_LOCAL_PATH.relative_to(PROJECT_ROOT)),
            "sha256": sha256sum(HA_LOCAL_PATH),
        }
    if openinverter_data:
        source_files["openinverter_gateway"] = {
            "path": str(OPENINVERTER_PATH.relative_to(PROJECT_ROOT)),
            "sha256": sha256sum(OPENINVERTER_PATH),
        }
    if inverter_to_mqtt_data:
        source_files["inverter_to_mqtt"] = {
            "path": str(INVERTER_TO_MQTT_PATH.relative_to(PROJECT_ROOT)),
            "sha256": sha256sum(INVERTER_TO_MQTT_PATH),
        }
    if grott_data:
        source_files["grott_register_layouts"] = {
            "path": str(GROTT_LAYOUTS_PATH.relative_to(PROJECT_ROOT)),
...
def main() -> None:
    ...
    parser.add_argument(
        "--schema",
        type=Path,
        default=PROJECT_ROOT / "knowledge" / "audit" / "consolidated-register-reference.schema.json",
