...
def _previous_h123(fresh_without_pipeline9: dict[str, Any], fresh_with_pipeline9: dict[str, Any]) -> dict[str, Any]:
    h123 = next(
        item
    ...
    return {
        ...
        "previous_pipeline9_rank": h123["rank"],
        "previous_pipeline9_expected_reduction": h123["expected_reduction"],
...
def build(starting_main_sha: str) -> tuple[dict[str, Any], list[dict[str, Any]]]:
    without = enumerate_candidates(include_pipeline9=False)
    enumeration = enumerate_candidates(include_pipeline9=True)
    ...
    before_decisions = accepted_decisions(include_pipeline9=True)
    authority = build_authority()
    ...
    previous_h123 = _previous_h123(without, enumeration)
    data = {
        ...
        "ranking_method": "PIPELINE-9 repository-wide enumeration with accepted PIPELINE-9 authority included; expected reduction uses unique canonical property cells and path evidence remains separate.",
...
def render(data: dict[str, Any]) -> str:
    ...
    lines += [
        ...
        "",
        f"The previous rank-1 H123 cohort is now accounted for; its fresh expected reduction is {data['previous_h123']['current_expected_reduction']}. The new rank-1 is `{delta['new_rank1']}`. Ranks 2+ shifted: `{delta['ranks_2_plus_shifted']}`.",
        "",
