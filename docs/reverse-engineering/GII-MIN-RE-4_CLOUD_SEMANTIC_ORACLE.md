...
capture, and one controlled response-injection experiment for the live MIN
...
GII-MIN-RE-3 smoke test. RE-4 records how the cloud oracle and Shine response
injection can be used for the remaining mapping work.
...
One field was technically tested, but not as a new semantic mapping:
...
## Proposed follow-up delta (not applied)

1. Establish a fresh cloud observation window first, using a confirmed portal
   update or a history timestamp that advances during the run. Keep every
   injected value stable for at least one complete five-minute Shine upload.
2. Use the completed GII-MIN-RE-3 I3000 mapping as the baseline; do not spend
   another experiment cycle rediscovering its `0..8` status smoke test.
3. Apply the same five-minute method to I3165, I3110, I3111, and I3211 only after the
   cloud window is proven fresh. Record Shine polling continuity separately
   from cloud semantic response.
4. Keep FC20 as a separate experiment matrix. Do not infer FC20 field offsets
   from ordinary I-register offsets.
5. Use the existing V4 history records as semantic context for BDC/BMS/grid
   fields, while requiring a fresh local/cloud pair before changing a physical
   register mapping.
