...
This document is retained as historical provenance from the HA research
checkout. The supported extracted paths and public-safety classifications are
now maintained in `README.md`, `sources/manifest.json` and `tools/README.md`.
...
| `testing/tcp_read.py` | small PyModbus TCP read, with a stale private example host | Useful template, not production-ready as written. |
