...
The following addresses are present in ``GrowattTLXH.cpp`` but do not appear in ``growatt_registers_best_guess.json``. Names are taken from the OpenInverterGateway source.
